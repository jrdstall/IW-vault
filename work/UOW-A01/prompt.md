...
- **If you have the `submit_result` MCP tool (Claude Desktop / Antigravity)**:
  ...
  *Note: Calling `submit_result` automatically writes `deliverable.md` into `vault/work/UOW-A01/` and moves the task to "Awaiting Review" on the Work Board. Jared does NOT need to copy files manually.*

- **If you do NOT have MCP tools (e.g. plain browser chat)**:
  Ensure the full approved Markdown report is in your chat response so Jared can manually save it to `vault/work/UOW-A01/deliverable.md`.